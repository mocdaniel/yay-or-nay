...
function aggregateSubmissionRatings(
  ...
  const result = fields.map((field) => {
    const { sum, count } = submissions.reduce(
      (acc, submission) => {
        const value = submission[field as keyof FormSubmission] as
          | number
          | null;
...
export default async function Page({
        ...
        <Card className="w-full max-w-md text-center">
          <CardHeader className="pb-4">
            ...
            <CardDescription className="text-slate-600">
              The feedback form you're looking for doesn't exist or may have
              been removed.
            </CardDescription>
          </CardHeader>
  ...
  const feedbackTypesPerFormType = {
    panel: ["topic", "expertise", "discussion", "panelists"] as unknown as [
      keyof FormSubmission
    ],
    talk: ["topic", "expertise", "presentation", "slides"] as unknown as [
      keyof FormSubmission
    ],
