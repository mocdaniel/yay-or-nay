...
export async function createEventAction(
  ...
  const validatedFormData = eventSchema.safeParse(Object.fromEntries(sanitizeFormData(formData)))
  ...
  const [event]: Event[] = await db`
    ...
    }, ${validatedFormData.data.date || null}, ${validatedFormData.data.location || null})
  RETURNING *
  `
...
export async function createFormAction(
  ...
  const validatedFormData = formSchema.safeParse(Object.fromEntries(sanitizeFormData(formData)))
